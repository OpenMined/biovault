...
test-integration-local cleanup="true" reset="false" wait="false":
	@echo "=== Starting Integration Test with Local Clients ==="
	@WAIT_THRESHOLD="{{wait}}" bash -uc 'threshold="$WAIT_THRESHOLD"; if [ -n "$threshold" ] && [ "$threshold" != "false" ]; then case "$threshold" in true|on|"" ) threshold=1 ;; *[!0-9]* ) threshold=1 ;; esac; echo "🔸 Wait prompts enabled starting at step ${threshold}"; echo "   Steps: 1=Hosts, 2=Start server, 3=Wait for server, 4=Start clients, 5=Run tests"; fi'
	@if [ "{{reset}}" = "true" ] || [ "{{reset}}" = "--reset" ]; then \
		echo "Reset requested, cleaning data..."; \
		just quick-reset; \
	fi
	...
	@WAIT_THRESHOLD="{{wait}}" STEP=1 LABEL="Step 1 complete: /etc/hosts configured for minio" bash -uc 'threshold="$WAIT_THRESHOLD"; step=$STEP; label="$LABEL"; if [ -z "$threshold" ] || [ "$threshold" = "false" ]; then exit 0; fi; case "$threshold" in true|on|"" ) threshold=1 ;; *[!0-9]* ) threshold=1 ;; esac; if [ "$step" -lt "$threshold" ]; then exit 0; fi; printf "\n[Wait %s] %s\nPress Enter to continue..." "$step" "$label" >/dev/tty; read -r _ </dev/tty; echo "" >/dev/tty'
	@echo ""
	@echo "2. Starting SyftBox server..."
	@just start-syftbox-server
	@WAIT_THRESHOLD="{{wait}}" STEP=2 LABEL="Step 2 complete: SyftBox server started" bash -uc 'threshold="$WAIT_THRESHOLD"; step=$STEP; label="$LABEL"; if [ -z "$threshold" ] || [ "$threshold" = "false" ]; then exit 0; fi; case "$threshold" in true|on|"" ) threshold=1 ;; *[!0-9]* ) threshold=1 ;; esac; if [ "$step" -lt "$threshold" ]; then exit 0; fi; printf "\n[Wait %s] %s\nPress Enter to continue..." "$step" "$label" >/dev/tty; read -r _ </dev/tty; echo "" >/dev/tty'
	@echo ""
	@echo "3. Waiting for server..."
	@just wait-for-server
	@WAIT_THRESHOLD="{{wait}}" STEP=3 LABEL="Step 3 complete: Server is ready" bash -uc 'threshold="$WAIT_THRESHOLD"; step=$STEP; label="$LABEL"; if [ -z "$threshold" ] || [ "$threshold" = "false" ]; then exit 0; fi; case "$threshold" in true|on|"" ) threshold=1 ;; *[!0-9]* ) threshold=1 ;; esac; if [ "$step" -lt "$threshold" ]; then exit 0; fi; printf "\n[Wait %s] %s\nPress Enter to continue..." "$step" "$label" >/dev/tty; read -r _ </dev/tty; echo "" >/dev/tty'
	@echo ""
	@echo "4. Starting local SyftBox clients with sbenv (including bad client)..."
	@just start-syftbox-all-clients-local
	@WAIT_THRESHOLD="{{wait}}" STEP=4 LABEL="Step 4 complete: Local clients started" bash -uc 'threshold="$WAIT_THRESHOLD"; step=$STEP; label="$LABEL"; if [ -z "$threshold" ] || [ "$threshold" = "false" ]; then exit 0; fi; case "$threshold" in true|on|"" ) threshold=1 ;; *[!0-9]* ) threshold=1 ;; esac; if [ "$step" -lt "$threshold" ]; then exit 0; fi; printf "\n[Wait %s] %s\nPress Enter to continue..." "$step" "$label" >/dev/tty; read -r _ </dev/tty; echo "" >/dev/tty'
	@echo ""
	@echo "5. Running integration tests..."
	@WAIT_THRESHOLD="{{wait}}" STEP=5 LABEL="Step 5 ready: Run integration test suite" bash -uc 'threshold="$WAIT_THRESHOLD"; step=$STEP; label="$LABEL"; if [ -z "$threshold" ] || [ "$threshold" = "false" ]; then exit 0; fi; case "$threshold" in true|on|"" ) threshold=1 ;; *[!0-9]* ) threshold=1 ;; esac; if [ "$step" -lt "$threshold" ]; then exit 0; fi; printf "\n[Wait %s] %s\nPress Enter to continue..." "$step" "$label" >/dev/tty; read -r _ </dev/tty; echo "" >/dev/tty'
	@if [ -n "$WAIT_THRESHOLD" ] && [ "$WAIT_THRESHOLD" != "false" ]; then export TEST_WAIT_FOR_STEPS="$WAIT_THRESHOLD"; fi; \
...
test-integration-local-inspect wait="false":
	just test-integration-local false false {{wait}}

# Run local integration test with reset
test-integration-local-reset wait="false":
	just test-integration-local true --reset {{wait}}
