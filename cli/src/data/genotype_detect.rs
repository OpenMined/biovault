...
pub fn detect_genotype_metadata(file_path: &str) -> Result<GenotypeMetadata> {
    ...
    let mut column_header_line: Option<String> = None;
    ...
    for line_result in &mut lines {
        ...
        if line.starts_with('#') || line.starts_with("//") {
            ...
            if data_lines.is_empty() && is_header_fields(&fields) {
                header_text.push_str(&line);
                header_text.push('\n');
                column_header_line = Some(line.clone());
                continue;
            }
    ...
    let source = detect_source(&header_lower, &data_lines, column_header_line.as_ref());
...
fn split_fields(line: &str) -> Vec<String> {
    // Strip inline comments (everything after #) but only if it's not the first character
    ...
    let delimiter = if clean_line.contains('\t') {
        '\t'
...
fn detect_source(
    header_lower: &str,
    data_lines: &[String],
    column_header_line: Option<&String>,
) -> Option<String> {
        ...
        || header_lower.contains("dynamicdnalabs")
    {
        ...
        || column_header_line.is_some_and(|line| is_decodeme_header(line))
        || data_lines.iter().any(|line| is_decodeme_header(line))
    {
        Some("deCODEme".to_string())
    } else if column_header_line.is_some_and(|line| is_familytreedna_header(line))
        || data_lines.iter().any(|line| is_familytreedna_header(line))
    {
...
mod tests {
    ...
    fn test_detect_source() {
        assert_eq!(
            detect_source("this is from 23andme", &[], None),
            Some("23andMe".to_string())
        );
        assert_eq!(
            detect_source("ancestrydna test", &[], None),
            Some("AncestryDNA".to_string())
        );
        assert_eq!(
            detect_source("genes for good data", &[], None),
            Some("Genes for Good".to_string())
        );
        assert_eq!(
            detect_source(
        ...
        assert_eq!(
            detect_source("data from ddna", &[], None),
            Some("Dynamic DNA".to_string())
        );
        assert_eq!(
            detect_source("https://dynamicdnalabs.com", &[], None),
            Some("Dynamic DNA".to_string())
        );
        assert_eq!(
            detect_source("# myheritage dna raw data", &[], None),
            Some("MyHeritage".to_string())
        );
        let decodeme_header = "Name,Variation,Chromosome,Position,Strand,YourCode".to_string();
        assert_eq!(
            detect_source("", &[], Some(&decodeme_header)),
            Some("deCODEme".to_string())
        );
        assert_eq!(
            detect_source("unknown source", &[], None),
            Some("Unknown".to_string())
        );
    }
