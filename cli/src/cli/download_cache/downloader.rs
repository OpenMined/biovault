...
impl DownloadCache {
    ...
    async fn download_file(
        ...
        if !head_response.status().is_success() {
            return Err(anyhow!("HTTP HEAD request failed: {}", head_response.status()));
        }
        ...
        let resume_from = if existing_size > 0 && accepts_ranges && total_size > 0 {
            if existing_size >= total_size {
                ...
                if show_progress {
                    println!("    ✓ File already fully downloaded ({} bytes)", existing_size);
                }
        ...
        if status == StatusCode::RANGE_NOT_SATISFIABLE {
            ...
            // Retry without range
            return self
                .download_file_fresh(url, target_path, show_progress, progress_callback, total_size)
                .await;
        }
        ...
        let (start_pos, content_length) = if status == StatusCode::PARTIAL_CONTENT {
            ...
            let content_len = response.content_length().unwrap_or(total_size - resume_from.unwrap_or(0));
