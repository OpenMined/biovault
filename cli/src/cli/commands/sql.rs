...
mod tests {
    ...
    // Helper to set up isolated test environment for integration-style tests
    ...
    // ========================================================================
    // Higher-level integration-style tests using public async API
    // ========================================================================

    /// Test: Full import → query → export → drop cycle
    #[test]
    #[serial]
    fn test_full_import_export_cycle() {
        ...
        runtime
            .block_on(async { drop("user_test_measurements".to_string(), true).await })
            .unwrap();
    ...
    fn test_import_with_column_mapping() {
        ...
        runtime
            .block_on(async { structure(Some("user_mapped_test".to_string()), false).await })
            .unwrap();
        ...
        // Cleanup
        runtime
            .block_on(async { drop("user_mapped_test".to_string(), true).await })
            .unwrap();

        teardown_test_env();
    }

    /// Test: Participant validation with different strategies
    #[test]
    #[serial]
    fn test_participant_validation_strategies() {
        ...
        runtime.block_on(async {
            let _ = drop("user_skip_test".to_string(), true).await;
            let _ = drop("user_create_test".to_string(), true).await;
        });

        teardown_test_env();
    }

    /// Test: Export formats (CSV, TSV, JSON)
    #[test]
    #[serial]
    fn test_export_formats() {
        ...
        // Cleanup
        runtime.block_on(async {
            let _ = drop("user_format_test".to_string(), true).await;
        });

        teardown_test_env();
    }

    /// Test: Protected table validation
    #[test]
    #[serial]
