...
pub async fn import_pipeline_with_deps(
    ...
    // Parse pipeline spec
    let spec: PipelineSpec = serde_yaml::from_str(&yaml_str)
    ...
    if !spec.steps.is_empty() {
        println!("\n{} Importing dependencies ({} steps):", "📦".cyan(), spec.steps.len());
        
        for (index, step) in spec.steps.iter().enumerate() {
            if let Some(uses) = &step.uses {
                // Skip absolute local paths
                if uses.starts_with('/') {
                    println!("   {} Step '{}' uses absolute path (keeping as-is)", "ℹ️ ".cyan(), step.id.dimmed());
                    continue;
                }
                
                // Skip if already a simple name
                if !uses.contains('/') && !uses.starts_with("http") {
                    println!("   {} Step '{}' already uses name (keeping as-is)", "ℹ️ ".cyan(), step.id.dimmed());
                    continue;
                }
        ...
        if any_rewritten {
            println!("\n{} Updating pipeline.yaml to use registered names...", "🔧".cyan());
            let updated_yaml = serde_yaml::to_string(&updated_spec)
                .context("Failed to serialize updated spec")?;
            fs::write(&pipeline_yaml_path, updated_yaml)?;
            println!("{} Pipeline YAML updated with registered names!", "✓".green());
        }
    }
    ...
    println!("\n{} Pipeline '{}' imported successfully!", "✅".green().bold(), pipeline_name.bold());
    println!("   Location: {}", pipeline_dir.display().to_string().dimmed());
