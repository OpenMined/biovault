...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python allele_freq_utils.py <command> [args]")
        print("Commands:")
        print("  load <npz> <index.json>  - Load and print stats")
        print("  to_tsv <npz> <index.json> <out.tsv>  - Convert to TSV")
        print("  from_tsv <in.tsv> <out.npz> <out_index.json>  - Convert from TSV")
        sys.exit(1)

    cmd = sys.argv[1]

    if cmd == "load":
        data = AlleleFreqData.load(sys.argv[2], sys.argv[3])
        print(f"Loaded {len(data)} loci")
        print(f"Total AC: {data.ac.sum()}")
        print(f"Total AN: {data.an.sum()}")
        print(f"Mean AF: {data.af.mean():.6f}")

    elif cmd == "to_tsv":
        data = AlleleFreqData.load(sys.argv[2], sys.argv[3])
        data.save_tsv(sys.argv[4])
        print(f"Wrote {len(data)} loci to {sys.argv[4]}")

    elif cmd == "from_tsv":
        data = AlleleFreqData.from_tsv(sys.argv[2])
        data.save_npz(sys.argv[3])
        data.index.save(sys.argv[4])
        print(f"Wrote {len(data)} loci to {sys.argv[3]} and {sys.argv[4]}")
