    ...
    def from_tsv(tsv_path: str) -> "AlleleFreqData":
        ...
        with open(tsv_path) as f:
            ...
            for line in f:
                ...
                rsids.append(parts[col_idx["rsid"]] if "rsid" in col_idx else "")
                ac.append(int(parts[ac_col]))
                an.append(int(parts[an_col]))
                if n_samples_col is not None:
                    n_samples.append(int(parts[n_samples_col]))
                elif num_homo_col is not None and num_hetero_col is not None:
                    n_samples.append(int(parts[num_homo_col]) + int(parts[num_hetero_col]))
                else:
                    n_samples.append(int(parts[an_col]) // 2)
