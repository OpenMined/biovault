...
class AlleleFreqData:
    ...
    def from_tsv(tsv_path: str) -> "AlleleFreqData":
        ...
        with open(tsv_path) as f:
            ...
            locus_col = col_idx.get("locus", col_idx.get("locus_key"))
            ac_col = col_idx.get("ac", col_idx.get("allele_count"))
            an_col = col_idx.get("an", col_idx.get("allele_number"))
            rsid_col = col_idx.get("rsid")
            n_samples_col = col_idx.get("n_samples")

            if locus_col is None:
                raise ValueError(
                    f"TSV missing required locus column ('locus' or 'locus_key'): {tsv_path}"
                )
            if ac_col is None:
                raise ValueError(
                    f"TSV missing required allele count column ('ac' or 'allele_count'): {tsv_path}"
                )
            if an_col is None:
                raise ValueError(
                    f"TSV missing required allele number column ('an' or 'allele_number'): {tsv_path}"
                )
            ...
            for line in f:
                ...
                ac_value = int(parts[ac_col])
                an_value = int(parts[an_col])

                if n_samples_col is not None and n_samples_col < len(parts):
                    n_samples_value = int(parts[n_samples_col])
                else:
                    # Backward compatibility for allele_freq TSVs that only provide
                    # allele_number: infer diploid sample count from AN.
                    n_samples_value = an_value // 2

                loci.append(locus)
                rsids.append(parts[rsid_col] if rsid_col is not None and rsid_col < len(parts) else "")
                ac.append(ac_value)
                an.append(an_value)
                n_samples.append(n_samples_value)
