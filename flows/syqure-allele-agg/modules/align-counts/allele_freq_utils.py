...
class AlleleFreqData:
    ...
    def from_tsv(tsv_path: str) -> "AlleleFreqData":
        ...
        with open(tsv_path) as f:
            ...
            ac_col = col_idx.get("allele_count") if "allele_count" in col_idx else col_idx.get("ac")
            an_col = col_idx.get("allele_number") if "allele_number" in col_idx else col_idx.get("an")
            n_samples_col = col_idx.get("n_samples")
            num_homo_col = col_idx.get("num_homo")
            num_hetero_col = col_idx.get("num_hetero")
            rsid_col = col_idx.get("rsid")

            if locus_col is None or ac_col is None or an_col is None:
                raise ValueError(
                    f"TSV missing required columns in {tsv_path}: "
                    f"locus_key/locus, allele_count/ac, allele_number/an"
                )
            ...
            for line in f:
                ...
                if not line:
                    continue
                parts = line.split('\t')
                max_req = max(locus_col, ac_col, an_col)
                if len(parts) <= max_req:
                    continue
                try:
                    locus_val = parts[locus_col]
                    ac_val = int(parts[ac_col])
                    an_val = int(parts[an_col])
                except (ValueError, IndexError):
                    continue
                loci.append(locus_val)
                rsids.append(parts[rsid_col] if rsid_col is not None and rsid_col < len(parts) else "")
                ac.append(ac_val)
                an.append(an_val)
                if n_samples_col is not None and n_samples_col < len(parts):
                    try:
                        n_samples.append(int(parts[n_samples_col]))
                    except ValueError:
                        n_samples.append(an_val // 2)
                elif (
                    num_homo_col is not None
                    and num_hetero_col is not None
                    and num_homo_col < len(parts)
                    and num_hetero_col < len(parts)
                ):
                    try:
                        n_samples.append(int(parts[num_homo_col]) + int(parts[num_hetero_col]))
                    except ValueError:
                        n_samples.append(an_val // 2)
                else:
                    n_samples.append(an_val // 2)
